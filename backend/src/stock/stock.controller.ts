...
export class StockController {
    ...
    @Roles('ADMIN', 'IT_ADMIN', 'COORDINATOR')
    @ApiOperation({ summary: 'Atualizar quantidade mínima de um item em uma carreta específica' })
    @ApiParam({ name: 'truckId' })
    @ApiParam({ name: 'stockItemId' })
    updateTruckItemMinimo(
        @Param('truckId') truckId: string,
        @Param('stockItemId') stockItemId: string,
        @Body() body: { novoMinimo: number },
        @Request() req: any,
    ) {
        if (typeof body.novoMinimo !== 'number' || body.novoMinimo < 0) {
            throw new BadRequestException('novoMinimo deve ser um número >= 0');
        }
        return this.stockService.updateTruckItemMinimo(truckId, stockItemId, body.novoMinimo, { id: req.user.id, role: req.user.role });
    }
    ...
    @ApiQuery({ name: 'stockItemId', required: false })
    @ApiQuery({ name: 'categoria', required: false, description: 'Filtrar por categoria do item (ex: CONSUMIVEL)' })
    ...
    listPurchaseRequests(
        ...
        return this.stockService.listPurchaseRequests(
            ...
            { status, stockItemId, categoria, onlyMine: onlyMine === 'true' },
        );
    }
