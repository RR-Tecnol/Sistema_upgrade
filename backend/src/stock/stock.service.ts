...
export class StockService {
    ...
    // ═══════════════════════════════════════════════════════════════════
    //   VERBAS — validação combinada (REQ 2026-05)
    // ═══════════════════════════════════════════════════════════════════

    /**
     * Valida as 3 camadas opcionais de verba antes de aprovar uma PR.
     * Qualquer camada configurada que for ultrapassada lança BadRequestException.
     * Se `bypass = true` (admin urgente), pula a validação das camadas B e C
     * mas mantém a camada A (valorTeto da própria PR) pois é contratual.
     *
     * Camada A — valorTeto da própria PR (campo na PR)
     * Camada B — StockBudget mensal por categoria
     * Camada C — AcaoStockBudget por Ação
     */
    private async validateBudgets(opts: {
        req: {
            id: string;
            stockItemId: string | null;
            categoria?: string;
            customCategoryId?: string | null;
            valorTotal: any;       // Prisma.Decimal
            valorTeto: any | null; // Prisma.Decimal | null
            acaoId: string | null;
        };
        approvedTotal: Prisma.Decimal;
        bypass?: boolean;
    }) {
        const { req, approvedTotal, bypass } = opts;

        // PR de verba (sem item específico) — a verba É o orçamento, não valida contra ela mesma
        if (!req.stockItemId) return;

        // ── Camada A: teto da própria PR ─────────────────────────────────
        if (req.valorTeto != null) {
            const teto = new Prisma.Decimal(req.valorTeto);
            if (approvedTotal.gt(teto)) {
                throw new BadRequestException(
                    `Valor aprovado (R$ ${approvedTotal.toFixed(2)}) ultrapassa o teto desta solicitação ` +
                    `(R$ ${teto.toFixed(2)}). Reduza a quantidade ou solicite um novo teto.`,
                );
            }
        }

        if (bypass) return; // admin ignorou verbas B e C

        const now = new Date();
        const ano = now.getFullYear();
        const mes = now.getMonth() + 1;

        // ── Camada B: Removida (A validação de Verba Mensal agora ocorre na Distribuição/Movimentação) ──

        // ── Camada C: verba por Ação ─────────────────────────────────────
        if (req.acaoId) {
            const acaoBudget = await this.prisma.acaoStockBudget.findUnique({
                where: { acaoId: req.acaoId },
            });
            if (acaoBudget) {
                const gastoAcaoAgg = await this.prisma.stockPurchaseRequest.aggregate({
                    where: {
                        id: { not: req.id },
                        acaoId: req.acaoId,
                ...
                const gastoAcao = new Prisma.Decimal(gastoAcaoAgg._sum.valorTotal ?? 0);
                const tetoAcao = new Prisma.Decimal(acaoBudget.valorTeto);
                if (gastoAcao.add(approvedTotal).gt(tetoAcao)) {
                    throw new BadRequestException(
                        `Verba da Ação esgotada: já foram aprovados R$ ${gastoAcao.toFixed(2)} ` +
                        `de R$ ${tetoAcao.toFixed(2)} disponíveis para esta ação. ` +
                        `Esta aprovação adicionaria mais R$ ${approvedTotal.toFixed(2)}.`,
                    );
                }
            }
        }
    ...
    /** Calcula o número de dias de consumo médio de um item (últimos 90 dias). */
    private async avgDailyConsumption(stockItemId: string): Promise<number> {
        const since = new Date();
        since.setDate(since.getDate() - 90);
        const agg = await this.prisma.stockMovement.aggregate({
            where: { stockItemId, type: 'SAIDA', createdAt: { gte: since } },
    ...
    async findAllItems(filters?: {
        ...
        if (filters?.onlyLow) {
            filtered = filtered.filter((i) => {
                const total =
                    Number(i.quantidadeAtual) +
                    (i.truckStocks as any[]).reduce(
                        (acc, ts) => acc + Number(ts.quantidadeAtual),
                        0,
                    );
                return total <= Number(i.quantidadeMinima);
            });
        }
        ...
        const pendingMap = new Map<string, number>(
            pendingByItem.filter(p => p.stockItemId != null).map((p) => [p.stockItemId!, p._count._all]),
        );
    ...
    async createItemWithPurchaseRequest(
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            // Auto-vincular verba se existir para esta categoria/mês
            let autoBudgetId = null;
            const budget = await this.findActiveBudgetForCategory(
                itemData.categoria as StockItemCategory,
                itemData.customCategoryId ?? undefined
            );
            if (budget) autoBudgetId = budget.id;
            ...
            // 💎 DEDUÇÃO DO SALDO PENDENTE NA VERBA (Provisionamento)
            // Se vinculamos a uma verba, devemos abater o valor da "PR de Provisionamento" (a que não tem item)
            // para que o total pendente na lista de solicitações não duplique o valor.
            if (autoBudgetId) {
                const mainPr = await tx.stockPurchaseRequest.findFirst({
                    where: { stockBudgetId: autoBudgetId, stockItemId: null, status: 'PENDENTE' }
                });
                if (mainPr) {
                    const novoValor = Prisma.Decimal.max(0, new Prisma.Decimal(mainPr.valorTotal).sub(valorTotal));
    ...
    async updateTruckItemMinimo(
        truckId: string,
        stockItemId: string,
        novoMinimo: number,
        actor: { id: string; role: UserRole },
    ) {
        const allowed: UserRole[] = ['ADMIN', 'IT_ADMIN', 'COORDINATOR'];
        if (!allowed.includes(actor.role)) {
            throw new ForbiddenException('Apenas Coordenadores e Administradores podem alterar o mínimo por carreta');
        }

        const record = await this.prisma.truckStockItem.findFirst({
            where: { truckId, stockItemId },
        });
        if (!record) throw new NotFoundException('Item não encontrado nesta carreta');

        return this.prisma.truckStockItem.update({
            where: { id: record.id },
            data: { quantidadeMinima: new Prisma.Decimal(novoMinimo) },
            include: {
                stockItem: {
    ...
    async createMovement(
        ...
        // ── Validação de Verba de Distribuição (StockBudget) ─────────
        if (dto.type === 'ENTRADA' && dto.toTruckId) {
            const now = new Date();
            const ano = now.getFullYear();
            const mes = now.getMonth() + 1;
            
            const budget = await this.prisma.stockBudget.findFirst({
                where: {
                    ano,
                    mes,
                    active: true,
                    OR: [
                        item.customCategoryId
                            ? { categoriaCustomId: item.customCategoryId }
                            : { categoriaEnum: item.categoria },
                        { categoriaEnum: item.categoria, categoriaCustomId: null },
                    ],
                },
            });
            ...
            if (budget) {
                ...
                // DEVOLUCAO (carreta → central) reverte o consumo — sem isso, uma devolução
                // não libera teto pra uma nova distribuição equivalente.
                ...
                const gastoAtualNum = movements.reduce((acc, m) => {
                ...
                // Mesma lógica do listStockBudgets: PRs aprovadas/recebidas da cat/mês
                // criadas ANTES desta verba e SEM stockBudgetId ampliam o teto efetivo.
                // Sem isso, gastos pré-verba bloqueiam distribuições legítimas mesmo
                // com saldo físico disponível na central.
                const prePRs = await this.prisma.stockPurchaseRequest.findMany({
                    where: {
                        stockBudgetId: null,
                        status: { in: ['APROVADA', 'RECEBIDA'] },
                        createdAt: { gte: mesInicio, lt: budget.createdAt },
                        stockItem: budget.categoriaEnum
                            ? { categoria: budget.categoriaEnum }
                            : { customCategoryId: budget.categoriaCustomId ?? undefined },
                    },
                ...
                const precoUnitarioNova = item.precoUnitario ? Number(item.precoUnitario) : 0;
                const valorNovaDist = Number(quantidade) * precoUnitarioNova;

                const gastoAtual = new Prisma.Decimal(gastoAtualNum);
                const tetoEfetivo = new Prisma.Decimal(budget.valorTeto).add(new Prisma.Decimal(valorPreVerba));

                if (gastoAtual.add(new Prisma.Decimal(valorNovaDist)).gt(tetoEfetivo)) {
                    throw new BadRequestException(
                        `Verba de distribuição da categoria esgotada: já foram distribuídos R$ ${gastoAtual.toFixed(2)} ` +
                        `de R$ ${tetoEfetivo.toFixed(2)} disponíveis em ${String(mes).padStart(2, '0')}/${ano}. ` +
                        `Esta transferência adicionaria mais R$ ${valorNovaDist.toFixed(2)}.`
                    );
                }
            }
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            switch (dto.type) {
                ...
                case 'AJUSTE': {
                    // AJUSTE usa SEMPRE modo SET: o valor informado é o NOVO saldo físico
                    // contado manualmente. O sistema calcula o delta e registra.
                    // (Modo DELTA foi removido — gerava confusão para perfis não técnicos.)
                    ajusteSnapshot.modo = 'SET';
                    const novoSaldoAbs = quantidade;
    ...
    private async findActiveBudgetForCategory(
        ...
        return this.prisma.stockBudget.findFirst({
            where: {
                mes,
                ano,
                categoriaEnum: customCategoryId ? null : categoria,
                categoriaCustomId: customCategoryId || null,
            },
        });
    }
    ...
    async createPurchaseRequest(
        ...
        // NOTA: removido o auto-link de StockBudget aqui. PRs criadas via fluxo de
        // insumo ficam independentes — só linkam à verba quando o front passar stockBudgetId
        // explicitamente (fluxo de "Nova Verba"). Isso evita que o modal de aprovação
        // mostre "Verba: X" pra uma PR que o admin nunca quis associar.
        ...
        const created = await this.prisma.stockPurchaseRequest.create({
            data: {
                ...
                precoUnitario,
                valorTotal,
                valorTeto: dto.valorTeto != null ? new Prisma.Decimal(dto.valorTeto) : null,
                acaoId: dto.acaoId ?? null,
                fornecedor: dto.fornecedor ?? null,
                urgente: dto.urgente ?? false,
                justificativa: dto.justificativa,
                comprovanteUrl: dto.comprovanteUrl ?? null,
                stockBudgetId: dto.stockBudgetId ?? null,
                requestedBy: actor.id,
            },
            include: {
                stockItem: { select: { id: true, nome: true, unidade: true } },
        ...
        // 💎 DEDUÇÃO DO SALDO PENDENTE NA VERBA (Provisionamento)
        if (dto.stockBudgetId) {
            const mainPr = await this.prisma.stockPurchaseRequest.findFirst({
                where: { stockBudgetId: dto.stockBudgetId, stockItemId: null, status: 'PENDENTE' }
            });
            if (mainPr) {
                const novoValor = Prisma.Decimal.max(0, new Prisma.Decimal(mainPr.valorTotal).sub(valorTotal));
                await this.prisma.stockPurchaseRequest.update({
                    where: { id: mainPr.id },
                    data: {
                        precoUnitario: novoValor,
                        valorTotal: novoValor,
    ...
    async listPurchaseRequests(
        ...
        if (filters?.stockItemId) where.stockItemId = filters.stockItemId;
        if (filters?.categoria) where.stockItem = { categoria: filters.categoria as StockItemCategory };
    ...
    async approvePurchaseRequest(
        ...
        const req = await this.prisma.stockPurchaseRequest.findUnique({
            ...
            include: {
                ...
                stockBudget: { select: { id: true, categoriaEnum: true, categoriaCustomId: true, mes: true, ano: true } },
            },
        });
        if (!req) throw new NotFoundException('Solicitação não encontrada');
        ...
        const isVerbaPR = !req.stockItemId && !!req.stockBudgetId;

        // Para verba PRs o admin escolhe item existente OU cria um novo inline
        let effectiveStockItemId: string | null = isVerbaPR ? (dto.stockItemId ?? null) : req.stockItemId;

        // Se newItemData foi enviado, criar o item ANTES da transação principal
        // (o item precisa existir para ser referenciado na PR e movimentação)
        if (isVerbaPR && dto.newItemData && !effectiveStockItemId) {
            const newItem = await this.prisma.stockItem.create({
                data: {
                    nome: dto.newItemData.nome,
                    unidade: dto.newItemData.unidade,
                    categoria: dto.newItemData.categoria,
                    fotoUrl: dto.newItemData.fotoUrl ?? null,
                    quantidadeAtual: 0,
                    quantidadeMinima: 0,
                    precoUnitario: dto.precoUnitario != null ? dto.precoUnitario : null,
                    fornecedor: dto.fornecedor ?? null,
                },
            });
            effectiveStockItemId = newItem.id;

            // Audit log da criação inline
            await this.auditLog.log({
                userId: actor.id,
                action: 'STOCK_ITEM_CREATE',
                tableName: 'stock_items',
                recordId: newItem.id,
                newData: this.snapshotItem(newItem),
            });
        }

        const effectivePrecoUnitario = isVerbaPR && dto.precoUnitario != null
            ? new Prisma.Decimal(dto.precoUnitario)
            : new Prisma.Decimal(req.precoUnitario as any);

        const approvedQtd = new Prisma.Decimal(dto.quantidade);

        if (approvedQtd.lte(0)) {
            throw new BadRequestException('A quantidade aprovada deve ser maior que zero');
        }

        if (isVerbaPR && !effectiveStockItemId) {
            throw new BadRequestException('Selecione o item a comprar ou forneça os dados para criar um novo item');
        }

        const reqQtdOriginal = new Prisma.Decimal(req.quantidade as any);
        const precoUnitario = effectivePrecoUnitario;
        const novoValorTotal = approvedQtd.mul(precoUnitario).toDecimalPlaces(2);

        let isPartial = false;
        let remainingQtd = new Prisma.Decimal(0);
        let remainingVerba = new Prisma.Decimal(0);

        if (isVerbaPR) {
            const baseValue = req.valorTeto ? new Prisma.Decimal(req.valorTeto as any) : new Prisma.Decimal(req.valorTotal as any);
            if (novoValorTotal.lt(baseValue)) {
                isPartial = true;
                remainingVerba = baseValue.sub(novoValorTotal);
            }
        } else {
            if (approvedQtd.lt(reqQtdOriginal)) {
                isPartial = true;
                remainingQtd = reqQtdOriginal.sub(approvedQtd);
            }
        }

        // Para verba PRs: validar que o total aprovado não excede o teto da verba
        if (isVerbaPR && req.stockBudgetId) {
            const budget = await this.prisma.stockBudget.findUnique({ where: { id: req.stockBudgetId } });
            if (budget && novoValorTotal.gt(new Prisma.Decimal(budget.valorTeto as any))) {
                throw new BadRequestException(
                    `Total aprovado (R$ ${novoValorTotal.toFixed(2)}) excede o teto da verba (R$ ${Number(budget.valorTeto).toFixed(2)})`,
                );
            }
        }

        // ── Validação de verbas (3 camadas) ─────────────────────────────
        await this.validateBudgets({
            req: {
                id: req.id,
                stockItemId: req.stockItemId,
                valorTotal: req.valorTotal,
                valorTeto: dto.valorTeto != null ? new Prisma.Decimal(dto.valorTeto) : req.valorTeto,
                acaoId: req.acaoId ?? null,
            },
            approvedTotal: novoValorTotal,
            bypass: dto.bypassBudgetCheck === true,
        });

        return this.prisma.$transaction(async (tx) => {
            // 1) Criar ContaPagar (PENDENTE) — com novoValorTotal e approvedQtd
            const dataVencimento = new Date();
            dataVencimento.setDate(dataVencimento.getDate() + 30);

            const fornecedorFinal = dto.fornecedor ?? req.fornecedor;
            const acaoSufixo = req.acaoId ? ` | Ação: ${req.acaoId.slice(0, 8)}` : '';

            // Para verba PRs: buscar nome do item escolhido pelo admin
            let verbaPrItemNome: string | null = null;
            if (isVerbaPR && effectiveStockItemId) {
                const vItem = await tx.stockItem.findUnique({ where: { id: effectiveStockItemId }, select: { nome: true } });
                verbaPrItemNome = vItem?.nome ?? null;
            }

            const descricaoCP = isVerbaPR
                ? `Verba ${req.stockBudget?.categoriaEnum ?? 'Estoque'} — Compra de ${verbaPrItemNome ?? 'item'}: ${approvedQtd} un × R$ ${precoUnitario.toFixed(2)} (PR ${req.id.slice(0, 8)})`
                : `Reposição de estoque: ${req.stockItem!.nome} — ${approvedQtd} un (solicitação ${req.id.slice(0, 8)})`;
            ...
            const conta = await tx.contaPagar.create({
                data: {
                    ...
                    status: 'pendente',
                    observacoes: `Solicitante: ${req.requestedBy} | Fornecedor: ${fornecedorFinal ?? '—'} | Aprovado por: ${actor.id}${acaoSufixo}`,
                },
            });

            // 2 + 3) Atualizar em-trânsito e criar movimentação ENCOMENDA
            // Para verba PRs: usa effectiveStockItemId (item escolhido pelo admin)
            let movementId: string | null = null;
            if (effectiveStockItemId) {
                const updateItemData: any = { quantidadeEmTransito: { increment: approvedQtd } };
                if (!isVerbaPR && dto.novoMinimo !== undefined && dto.novoMinimo !== null) {
                    updateItemData.quantidadeMinima = new Prisma.Decimal(dto.novoMinimo);
                }
                await tx.stockItem.update({ where: { id: effectiveStockItemId }, data: updateItemData });

                const movement = await tx.stockMovement.create({
                    data: {
                        type: 'ENCOMENDA',
                        stockItemId: effectiveStockItemId,
                        quantidade: approvedQtd,
            ...
            // 4) Atualizar a solicitação original (modificada se for parcial)
            const updated = await tx.stockPurchaseRequest.update({
                where: { id },
                data: {
                    // Para verba PRs: vincular o item escolhido pelo admin e ajustar preço
                    ...(isVerbaPR && effectiveStockItemId ? { stockItemId: effectiveStockItemId } : {}),
                    precoUnitario: precoUnitario,
                    quantidade: approvedQtd,
                    valorTotal: novoValorTotal,
                    valorTeto: dto.valorTeto != null ? new Prisma.Decimal(dto.valorTeto) : undefined,
            ...
            // 5) Se for parcial, criar nova solicitação PENDENTE para a diferença
            let newPendingPr = null;
            if (isPartial) {
                newPendingPr = await tx.stockPurchaseRequest.create({
                    data: {
                        stockItemId: isVerbaPR ? null : req.stockItemId,
                        stockBudgetId: req.stockBudgetId,
                        quantidade: isVerbaPR ? new Prisma.Decimal(1) : remainingQtd,
                        precoUnitario: isVerbaPR ? remainingVerba : req.precoUnitario,
                        valorTotal: isVerbaPR ? remainingVerba : remainingQtd.mul(precoUnitario).toDecimalPlaces(2),
                        valorTeto: isVerbaPR ? remainingVerba : undefined,
                        fornecedor: req.fornecedor,
                        urgente: req.urgente,
                        justificativa: isVerbaPR
                            ? `💰 Saldo restante desta verba (Origem: #${req.id.slice(0, 8)}).\nEsta solicitação continua aberta para uso porque o valor total da verba ainda não foi totalmente consumido.\n\nJustificativa original: ${req.justificativa ?? '—'}`
                            : `📦 Saldo remanescente desta solicitação (Origem: #${req.id.slice(0, 8)}).\nO item permanece na lista porque a quantidade aprovada foi apenas uma parte do total solicitado originalmente.\n\nJustificativa original: ${req.justificativa ?? '—'}`,
                        requestedBy: req.requestedBy,
                    }
                });
            }

            return { updated, newPendingPr };
        }).then(async (result) => {
            const { updated, newPendingPr } = result;
            ...
            if (newPendingPr) {
                await this.auditLog.log({
                    ...
                    action: 'STOCK_PURCHASE_REQUEST_CREATE',
                    tableName: 'stock_purchase_requests',
                    recordId: newPendingPr.id,
                    newData: {
                        id: newPendingPr.id,
                        stockItemId: newPendingPr.stockItemId,
                        quantidade: newPendingPr.quantidade?.toString?.() ?? newPendingPr.quantidade,
                        precoUnitario: newPendingPr.precoUnitario?.toString?.() ?? newPendingPr.precoUnitario,
                        valorTotal: newPendingPr.valorTotal?.toString?.() ?? newPendingPr.valorTotal,
                        status: newPendingPr.status,
                        nota: 'Gerado automaticamente por aprovação parcial',
                    }
                });
            }
    ...
    async confirmReceiptOfPurchase(
        ...
        const isVerbaPR = !req.stockItemId;

        const updated = await this.prisma.$transaction(async (tx) => {
            let movReposicaoId: string | null = null;

            if (!isVerbaPR && req.stockItemId) {
                ...
                // 3) Cria StockMovement REPOSICAO
                ...
                movReposicaoId = movReposicao.id;
            }
    ...
    async rejectPurchaseRequest(
        ...
        // 💎 DEVOLUÇÃO DO SALDO PARA A VERBA (Provisionamento)
        if (req.stockBudgetId) {
            const mainPr = await this.prisma.stockPurchaseRequest.findFirst({
                where: { stockBudgetId: req.stockBudgetId, stockItemId: null, status: 'PENDENTE' }
            });
            if (mainPr) {
                const novoValor = new Prisma.Decimal(mainPr.valorTotal).add(req.valorTotal);
                await this.prisma.stockPurchaseRequest.update({
                    where: { id: mainPr.id },
                    data: {
                        precoUnitario: novoValor,
    ...
    async lowStockAlerts(severity: 'low' | 'critical' = 'low') {
        ...
        return items.filter((i) => {
            const central = Number(i.quantidadeAtual);
            const carretas = (i.truckStocks as any[]).reduce(
                (acc, ts) => acc + Number(ts.quantidadeAtual),
                0,
            );
            const total = central + carretas;
            const minimo = Number(i.quantidadeMinima);

            if (minimo <= 0) return false; // sem mínimo definido, não há alerta
            if (severity === 'critical') return total <= minimo * 0.5;
            return total <= minimo;
        });
    }
    ...
    async dashboard() {
        const [
            ...
            this.prisma.stockItem.findMany({
                ...
                select: {
                    quantidadeAtual: true,
                    precoUnitario: true,
                    truckStocks: { select: { quantidadeAtual: true } },
                },
            }),
            this.prisma.stockItem.findMany({
        ...
        const valorTotal = valorTotalEstimado.reduce((acc, i) => {
            const central = Number(i.quantidadeAtual);
            const carretas = (i.truckStocks as any[]).reduce(
                (t, ts) => t + Number(ts.quantidadeAtual),
                0,
            );
            return acc + (central + carretas) * Number(i.precoUnitario ?? 0);
        }, 0);
    ...
    async financialDashboard() {
        ...
        const budgets = await this.prisma.stockBudget.findMany({
            where: { ano, mes, active: true },
        });
        const totalVerba = budgets.reduce((acc, b) => acc + Number(b.valorTeto), 0);

        // Consumo da verba = soma do valor dos insumos das categorias com verba
        // que foram DISTRIBUÍDOS para carretas no mês (StockMovement ENTRADA com
        // toTruckId != null). Valor = quantidade × precoUnitario do item.
        // Mesma fonte de verdade usada em listStockBudgets — evita o double-count
        // que existia quando uma PR mãe aprovada somava com os movimentos.
        const orFilters: any[] = budgets.map(b => ({
            stockItem: b.categoriaEnum
                ? { categoria: b.categoriaEnum }
                : { customCategoryId: b.categoriaCustomId ?? undefined },
        }));

        // Inclui DEVOLUCAO (carreta → central) pra reverter consumo da verba.
        // OR de tipo precisa coexistir com o OR de categoria — usamos AND explícito.
        const movements = budgets.length === 0 ? [] : await this.prisma.stockMovement.findMany({
            where: {
                AND: [
                    {
                        OR: [
    ...
    async upsertReservationsForAcao(
        ...
        // Valida saldo disponível no ato da reserva (central + em trânsito)
        // Soma: reservas existentes de OUTRAS ações + o que estamos pedindo agora.
        // Bloqueia para alertar o admin antes de chegar no dia da ação.
        const alertasSaldoInsuficiente: string[] = [];
        for (const it of items) {
            const stockItem = await this.prisma.stockItem.findUnique({
                where: { id: it.stockItemId },
                select: { id: true, nome: true, unidade: true, quantidadeAtual: true, quantidadeEmTransito: true },
            });
            if (!stockItem) continue;
            const disponivel = new Prisma.Decimal(stockItem.quantidadeAtual).add(
                new Prisma.Decimal(stockItem.quantidadeEmTransito),
            );
            // Total reservado em OUTRAS ações para este item
            const outrasReservasAgg = await this.prisma.acaoStockReservation.aggregate({
                where: { stockItemId: it.stockItemId, acaoId: { not: acaoId } },
                _sum: { quantidadePrevista: true },
            });
            const outrasReservas = new Prisma.Decimal(outrasReservasAgg._sum.quantidadePrevista ?? 0);
            const necessario = new Prisma.Decimal(it.quantidadePrevista);
            if (disponivel.sub(outrasReservas).lt(necessario)) {
                alertasSaldoInsuficiente.push(
                    `"${stockItem.nome}": necessário ${necessario} ${stockItem.unidade}, ` +
                    `disponível líquido ${disponivel.sub(outrasReservas).toFixed(3)} ${stockItem.unidade} ` +
                    `(saldo ${disponivel.toFixed(3)} − outras reservas ${outrasReservas.toFixed(3)})`,
                );
            }
        }
        if (alertasSaldoInsuficiente.length > 0) {
            throw new BadRequestException(
                `Saldo insuficiente para ${alertasSaldoInsuficiente.length} item(s) do kit. ` +
                `Solicite compra ou reduza o previsto antes de salvar:\n` +
                alertasSaldoInsuficiente.join('\n'),
            );
        }
    ...
    async listStockBudgets(filters?: { ano?: number; mes?: number; includeInactive?: boolean }) {
        ...
        // Consumo da verba = soma do valor dos insumos daquela categoria
        // DISTRIBUÍDOS para as carretas no período (StockMovement ENTRADA
        // com toTruckId != null). Valor = quantidade × precoUnitario do item.
        // Pode estourar o teto (>100%); o frontend mostra o estado vermelho.
        const budgetsWithConsumed = await Promise.all(
            budgets.map(async (b) => {
                // DEVOLUCAO (carreta → central) reverte o consumo da verba — uma distribuição
                // que voltou pra central não deve continuar contabilizada como gasta.
                const movements = await this.prisma.stockMovement.findMany({
                    where: {
                        OR: [
                            { type: 'ENTRADA', toTruckId: { not: null } },
                ...
                // Gastos "pré-verba": PRs aprovadas/recebidas da mesma categoria/mês
                // que NÃO foram criadas via verba e que aconteceram ANTES desta verba.
                // Somamos no teto exibido para que a barra reflita o orçamento real do mês.
                const prePRs = await this.prisma.stockPurchaseRequest.findMany({
                    where: {
                        stockBudgetId: null,
                        status: { in: ['APROVADA', 'RECEBIDA'] },
    ...
    async createStockBudget(dto: CreateStockBudgetDto, actor: { id: string; role: UserRole }) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (budget) {
                // Agrega o total à verba existente
                const novoTeto = Number(budget.valorTeto) + dto.valorTeto;
                budget = await tx.stockBudget.update({
                    where: { id: budget.id },
                    data: {
                        valorTeto: new Prisma.Decimal(novoTeto),
                    }
                });

                // Atualiza a PR de verba "mãe" (a que não tem item e está pendente)
                const mainPr = await tx.stockPurchaseRequest.findFirst({
                    where: { stockBudgetId: budget.id, stockItemId: null, status: 'PENDENTE' }
                });
                ...
                if (mainPr) {
                    ...
                    // Se não houver PR pendente (já foi toda aprovada), cria uma nova para o valor adicional
                ...
                await tx.stockPurchaseRequest.create({
                    data: {
                        ...
                        precoUnitario: new Prisma.Decimal(dto.valorTeto),
                        valorTotal: new Prisma.Decimal(dto.valorTeto),
                        justificativa: `Verba ${catLabel} — ${MESES_PT[dto.mes - 1]}/${dto.ano}. Administrador decide quantidade e itens na aprovação.`,
                        requestedBy: actor.id,
                        urgente: false,
                    },
                });
    ...
    async getAcaoStockBudget(acaoId: string) {
        return this.prisma.acaoStockBudget.findUnique({
            where: { acaoId },
    ...
    /** Retorna o consumo real (R$) de uma ação nos PRs APROVADAS/RECEBIDAS. */
    async getAcaoStockBudgetStatus(acaoId: string) {
        const budget = await this.prisma.acaoStockBudget.findUnique({ where: { acaoId } });
        const gastoAgg = await this.prisma.stockPurchaseRequest.aggregate({
            where: { acaoId, status: { in: ['APROVADA', 'RECEBIDA'] } },
            _sum: { valorTotal: true },
        });
        const gasto = new Prisma.Decimal(gastoAgg._sum.valorTotal ?? 0);
        return {
            acaoId,
            temVerba: budget != null,
            valorTeto: budget ? Number(budget.valorTeto) : null,
            valorGasto: Number(gasto),
            valorDisponivel: budget ? Number(new Prisma.Decimal(budget.valorTeto).sub(gasto)) : null,
            percentualUsado: budget ? Number(gasto.div(new Prisma.Decimal(budget.valorTeto)).mul(100).toDecimalPlaces(1)) : null,
        };
    }
}
