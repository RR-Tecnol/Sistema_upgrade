...
export class EnrollmentsService {
    ...
    async create(createEnrollmentDto: CreateEnrollmentDto, meta?: PublicEnrollmentRequestMeta) {
        ...
        // ── EMAIL: confirmar recebimento da inscrição ao aluno ────────────────
        // Fire-and-forget — nunca bloqueia a resposta ao aluno
        try {
            const studentEmail = (enrollment.student as any)?.contact?.email
                              ?? enrollment.student?.user?.email;
            const studentName  = enrollment.student?.user?.name;
            const courseName   = (enrollment.class as any)?.course?.name || 'Curso';
            if (studentEmail && studentName) {
                void this.mail.sendEnrollmentReceived(studentEmail, studentName, courseName, protocol);
            }
    ...
    async approve(id: string, userId: string, notes?: string) {
        ...
        // ── EMAIL: avisar aluno que sua inscrição foi aprovada ────────────────
        try {
            const studentEmail = (updated.student as any)?.contact?.email
            ...
            if (studentEmail && studentName) {
                void this.mail.sendEnrollmentApproved(studentEmail, studentName, courseName, startDate, city);
            }
        } catch { /* email nunca bloqueia */ }
    ...
    async reject(id: string, userId: string, rejectionReason: string) {
        ...
        // ── EMAIL: avisar aluno que sua inscrição foi rejeitada ────────────────
        try {
            const studentEmail = (updated.student as any)?.contact?.email
                              ?? updated.student?.user?.email;
            const studentName  = updated.student?.user?.name;
