 ...
 *   - layout GSR                     → initialStockItemId / initialTruckId (alias dos defaults)
 */
export function MovimentacaoModal({
    ...
    defaultToTruckId,
    defaultAcaoId,
    /** Alias de defaultItemId — tabela GSR / carreta */
    initialStockItemId,
    /** Alias de defaultFromTruckId — saída/baixa a partir desta carreta */
    initialTruckId,
    onSuccess,
}: {
    open: boolean;
    ...
    const resolvedItemId = defaultItemId ?? initialStockItemId;
    const resolvedFromTruck = defaultFromTruckId ?? initialTruckId;

    const [type, setType] = useState<StockMovementType>(defaultType);
    const [stockItemId, setStockItemId] = useState(resolvedItemId ?? '');
    const [fromTruckId, setFromTruckId] = useState(resolvedFromTruck ?? '');
    ...
    useEffect(() => {
        if (!open) return;
        setType(defaultType);
        setStockItemId((defaultItemId ?? initialStockItemId) ?? '');
        setFromTruckId((defaultFromTruckId ?? initialTruckId) ?? '');
        ...
        setAjusteMode('SET');
        setError(null);
    }, [open, defaultType, defaultItemId, initialStockItemId, defaultFromTruckId, initialTruckId, defaultToTruckId, defaultAcaoId]);
            ...
            <div
                ...
                <div
                    ...
                    <div className="custom-scrollbar" style={{ padding: '1.1rem 1.25rem', overflowY: 'auto', flex: 1 }}>
                        ...
                        {/* AJUSTE: instrução de contagem física (modo SET fixo) */}
                        {type === 'AJUSTE' && (
                            <div style={{
                                ...
                                <div style={{ fontSize: '0.72rem', color: '#78350F', lineHeight: 1.5 }}>
                                    <strong>Contagem física:</strong> conte manualmente quantas unidades existem e
                                    digite o total abaixo. O sistema calcula o ajuste automaticamente.
                                </div>
                            </div>
                        )}
                        ...
                        <Field
                            label={
                                type === 'AJUSTE'
                                    ? `Novo saldo real (${selectedItem?.unidade ?? 'un'}) — quanto TEM`
                            ...
                            />
                            {selectedItem?.precoUnitario && qtdNum > 0 && type !== 'AJUSTE' && (
                                <div style={{ fontSize: '0.75rem', color: '#64748B', marginTop: 6, fontWeight: 500 }}>
                                    Valor total estimado: <strong style={{ color: '#059669', fontFamily: 'Orbitron, sans-serif' }}>{(qtdNum * Number(selectedItem.precoUnitario)).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</strong>
                                </div>
                            )}
                        </Field>
                            ...
                            <div style={{
                                ...
                                <div style={{ fontSize: '0.86rem', fontWeight: 700, color: '#0F172A', display: 'flex', alignItems: 'center', gap: 6 }}>
                                    ...
                                    <span style={{ fontSize: '0.72rem', color: '#64748B' }}>
                                        {formatItemQuantity(selectedItem, 1).replace('1 ', '')}
                                    </span>
                                ...
                                {/* Rótulo de conteúdo quando disponível */}
                                {selectedItem.conteudoQuantidade && selectedItem.conteudoUnidade && qtdNum > 0 && (
                                    <div style={{ fontSize: '0.7rem', color: '#475569', marginTop: 3, fontFamily: 'JetBrains Mono, monospace' }}>
                                        {formatItemQuantity(selectedItem, qtdNum, { showTotal: true })}
                                    </div>
                                )}
                                {preview.depois < 0 && (
                                ...
                                {type === 'ENTRADA' && (
                                    <div style={{ fontSize: '0.7rem', color: '#475569', marginTop: 4 }}>
                                        Esta movimentação também credita {formatItemQuantity(selectedItem, qtdNum)} na carreta destino.
                                    </div>
                                )}
                                {type === 'DEVOLUCAO' && (
                                    <div style={{ fontSize: '0.7rem', color: '#475569', marginTop: 4 }}>
                                        Esta movimentação também credita {formatItemQuantity(selectedItem, qtdNum)} no estoque central.
                                    </div>
                                )}
                        ...
                        {/* Alerta de consumo anormal (SAIDA > 50% saldo ou quantidade grande) */}
                        {type === 'SAIDA' && selectedItem && saldoEscopo && qtdNum > 0 && saldoEscopo.valor > 0 && qtdNum >= saldoEscopo.valor * 0.5 && (
                            <div style={{
                                padding: '0.65rem 0.9rem', borderRadius: 10,
                                ...
                                <div>
                                    <div style={{ fontSize: '0.72rem', fontWeight: 800, color: '#92400E', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                        Consumo acima do normal
                                    </div>
                                    <div style={{ fontSize: '0.7rem', color: '#78350F', lineHeight: 1.5 }}>
                                        Você está baixando <strong>{formatItemQuantity(selectedItem, qtdNum)}</strong>, que representa{' '}
                                        <strong>{Math.round((qtdNum / saldoEscopo.valor) * 100)}%</strong> do saldo disponível ({saldoEscopo.valor} {selectedItem.unidade}).
