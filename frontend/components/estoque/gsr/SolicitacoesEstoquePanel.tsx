...
export function SolicitacoesEstoquePanel({ highlightPrId, initialStatusTab, initialCategoria, onStockUpdated, refreshKey, readOnly = false }: Props) {
    ...
    const openReview = (req: StockPurchaseRequest, mode: 'approve' | 'reject') => {
        ...
        resetVerbaPrState();
        if (mode === 'approve') {
            if (!req.stockBudget || !!req.stockItemId) {
                ...
                const catEnum = req.stockBudget?.categoriaEnum as StockItemCategory | undefined;
                const catCustom = req.stockBudget?.categoriaCustomId ?? undefined;
    ...
    const handleReview = async () => {
        ...
        try {
            if (reviewMode === 'approve') {
                const isVerbaReview = !!reviewing.stockBudget;
                if (isVerbaReview) {
                    ...
                    const approveData: any = {
                        ...
                        fornecedor: reviewSupplier.trim() || undefined,
                    ...
                    if (verbaPrIsNewItem) {
                        approveData.newItemData = {
                            ...
                            categoria: reviewing.stockBudget?.categoriaEnum ?? 'OUTRO',
                    ...
                    await stockApi.purchaseRequests.approve(reviewing.id, {
                        reviewNote: reviewNote.trim() || undefined,
                        quantidade: Number(reviewQuantity),
                        novoMinimo: updateMin && reviewMinimo !== '' ? Number(reviewMinimo) : undefined,
                        fornecedor: reviewSupplier.trim() || undefined,
                    });
                }
                        ...
                        <div key={req.id} id={`pr-card-${req.id}`}>
                            <RequestCard
                                ...
                                onConfirmReceipt={async () => {
                                    const recDesc = req.stockBudget
                                        ? `verba de "${req.stockBudget.categoriaEnum ?? 'Categoria personalizada'}"`
                    ...
                    <div className="modal-overlay" onClick={closeReview}>
                        <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 520 }}>
                            ...
                            <p style={{ fontSize: '0.85rem', color: '#64748B' }}>
                                {reviewing.stockBudget
                                    ? `Verba: ${reviewing.stockBudget.categoriaEnum ?? 'Categoria personalizada'} — ${['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][reviewing.stockBudget.mes - 1]}/${reviewing.stockBudget.ano}`
                                    : `${reviewing.stockItem?.nome ?? 'Item'} · ${Number(reviewing.quantidade)} ${reviewing.stockItem?.unidade ?? ''}`}
                            </p>
                            {reviewMode === 'approve' && reviewing.stockBudget && !reviewing.stockItemId && (() => {
                                const teto = Number(reviewing.stockBudget.valorTeto ?? reviewing.valorTotal);
        ...
        >
            ...
            <div style={{ padding: '1rem 1.1rem' }}>
                ...
                {req.stockItemId ? (
                    ...
                    const cat = resolveCategoria(req.stockBudget);
                    return (
                        <div>
                            <div style={{ fontWeight: 800, color: '#0F172A', fontSize: '0.95rem' }}>
                                {cat.label}
                                {' — '}
                                {['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][req.stockBudget.mes - 1]}/{req.stockBudget.ano}
                            </div>
                ...
                {(!req.stockBudget || !!req.stockItemId) && (
