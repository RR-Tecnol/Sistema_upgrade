...
export function SolicitarCompraModal({
    ...
    onClose: () => void;
    onSuccess?: () => void;
    /** Orçamento da verba de origem — exibido como referência/cap */
    verbaBudget?: number;
}) {
    ...
    const [atualizarMinimo, setAtualizarMinimo] = useState(false);
    const [novoMinimo, setNovoMinimo] = useState<number | ''>('');
    ...
    useEffect(() => {
        if (item) {
            ...
            setUrgente(false);
            setError(null);
            setAtualizarMinimo(false);
            setNovoMinimo(Number(item.quantidadeMinima) || '');
        }
    }, [item]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Atualizar mínimo se solicitado
            if (atualizarMinimo && typeof novoMinimo === 'number' && novoMinimo >= 0) {
                await stockApi.items.update(item.id, { quantidadeMinima: novoMinimo });
            }
                ...
                <div
                    ...
                    <div className="custom-scrollbar" style={{ padding: '1.1rem 1.25rem', overflowY: 'auto', flex: 1 }}>
                        ...
                        <div style={{
                            ...
                            border: verbaBudget != null && valorTotal > verbaBudget && verbaBudget > 0
                                ? '1.5px solid #FCA5A5'
                                : '1.5px solid #FEF08A',
                            marginBottom: verbaBudget != null && verbaBudget > 0 ? 6 : 14,
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 900, fontSize: '1.45rem', lineHeight: 1.1, marginTop: 2, color: verbaBudget != null && valorTotal > verbaBudget && verbaBudget > 0 ? '#DC2626' : '#7C5A00' }}>
                        ...
                        {verbaBudget != null && verbaBudget > 0 && (
                            <div style={{
                                ...
                                background: valorTotal > verbaBudget ? '#FEF2F2' : '#F0FDF4',
                                border: `1px solid ${valorTotal > verbaBudget ? '#FCA5A5' : '#BBF7D0'}`,
                                display: 'flex', alignItems: 'center', gap: 8,
                                fontSize: '0.75rem', fontWeight: 700,
                                color: valorTotal > verbaBudget ? '#DC2626' : '#15803D',
                            }}>
                                <span>{valorTotal > verbaBudget ? '⚠️' : '✅'}</span>
                                <span>
                                    Verba disponível: <strong>R$ {verbaBudget.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
                                    {valorTotal > verbaBudget
                                        ? ` — excede em R$ ${(valorTotal - verbaBudget).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                                        : ` — dentro do orçamento (sobra R$ ${(verbaBudget - valorTotal).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`
                                    }
                                </span>
                            </div>
                        )}
                        ...
                        <div style={{
                            ...
                            border: `1px solid ${atualizarMinimo ? '#FFD600' : '#E5E7EB'}`,
                            background: atualizarMinimo ? '#FFFDE7' : '#F9FAFB',
                            marginBottom: 10,
                        }}>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                                <input
                                    type="checkbox"
                                    checked={atualizarMinimo}
                                    onChange={e => setAtualizarMinimo(e.target.checked)}
                                    style={{ accentColor: '#B89B00' }}
                                />
                                <span style={{ fontSize: '0.78rem', fontWeight: 700, color: atualizarMinimo ? '#B89B00' : '#475569' }}>
                                    📐 Atualizar mínimo de reposição deste item
                                </span>
                            </label>
                            {atualizarMinimo && (
                                <div style={{ marginTop: 10 }}>
                                    <label style={{ display: 'block', fontSize: '0.62rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#B89B00', marginBottom: 5 }}>
                                        Novo mínimo ({item.unidade})
                                    </label>
                                    <input
                                        type="number"
                                        min={0}
                                        step={0.001}
                                        value={novoMinimo}
                                        onChange={e => setNovoMinimo(e.target.value === '' ? '' : Number(e.target.value))}
                                        style={{
                                            ...inputStyle,
                                            borderColor: '#FFD600',
                                            width: 140,
                                    ...
                                    />
                                    <small style={{ ...hintStyle, marginTop: 4 }}>
                                        Mínimo atual: {Number(item.quantidadeMinima)} {item.unidade}
                                    </small>
                                </div>
                            )}
                        </div>
                        ...
                        <div style={{
                            ...
                            esta solicitação vai para <Link href="/admin/estoque?tab=solicitacoes" style={{ color: '#1D4ED8', fontWeight: 700 }}>Solicitações de Compra</Link>{' '}
