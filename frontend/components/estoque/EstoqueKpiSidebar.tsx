...
const META: Record<EstoqueKpiKey, SidebarMeta> = {
    totalAtivos: {
        ...
        quickActions: [
            { href: '/admin/estoque/itens',       label: 'Ver lista completa',      icon: '📋' },
            { href: '/admin/estoque/itens/novo',  label: 'Cadastrar novo item',     icon: '➕' },
            { href: '/admin/estoque/historico?action=STOCK_ITEM', label: 'Auditoria de itens', icon: '📜' },
        ],
    },
    saldoCarretas: {
        ...
        quickActions: [
            ...
            { href: '/admin/estoque/movimentacoes', label: 'Movimentações entre carretas', icon: '🔄' },
            { href: '/admin/estoque/historico?action=TRANSFERENCIA', label: 'Histórico de transferências', icon: '📜' },
        ],
    },
    estoqueCritico: {
        ...
        quickActions: [
            { href: '/admin/estoque/itens?onlyLow=true', label: 'Ver itens com alerta', icon: '🚨' },
            { href: '/admin/estoque/solicitacoes',       label: 'Abrir solicitações urgentes', icon: '🔥' },
            { href: '/admin/estoque/historico?action=REPOSICAO', label: 'Histórico de reposições', icon: '📜' },
        ],
    },
    estoqueBaixo: {
        title: 'Estoque baixo',
        ...
        quickActions: [
            { href: '/admin/estoque/itens?onlyLow=true', label: 'Ver itens com alerta', icon: '🚨' },
            { href: '/admin/estoque/solicitacoes',       label: 'Solicitações de compra', icon: '🛒' },
            { href: '/admin/estoque/historico?action=REPOSICAO', label: 'Histórico de reposições', icon: '📜' },
        ],
    },
    vencendo: {
        ...
        quickActions: [
            { href: '/admin/estoque/itens?onlyExpiring=true', label: 'Ver itens vencendo', icon: '⏰' },
            { href: '/admin/estoque/historico?action=PERDA',  label: 'Histórico de perdas', icon: '📜' },
        ],
    },
    solicitacoesPendentes: {
        ...
        quickActions: [
            { href: '/admin/estoque/solicitacoes', label: 'Analisar solicitações', icon: '🛒' },
            { href: '/admin/estoque/historico?action=PURCHASE_REQUEST', label: 'Histórico de aprovações', icon: '📜' },
    ...
    emTransito: {
        ...
        quickActions: [
            { href: '/admin/estoque/solicitacoes?status=APROVADA', label: 'Ver encomendas em trânsito', icon: '📦' },
            { href: '/admin/contas-a-pagar?tipo=estoque_reposicao', label: 'Pagar contas a pagar de estoque', icon: '💰' },
            { href: '/admin/estoque/historico?action=ENCOMENDA',   label: 'Histórico de encomendas',   icon: '📜' },
        ],
    },
    movimentacoesMes: {
        ...
        quickActions: [
            { href: '/admin/estoque/movimentacoes', label: 'Ver movimentações', icon: '🔄' },
            { href: '/admin/estoque/historico',     label: 'Auditoria completa', icon: '📜' },
        ],
    },
};
                                                ...
                                                <Link key={it.id} href={`/admin/estoque/itens/${it.id}`} style={{
                                                    ...
                                                    <div style={{ flex: 1, minWidth: 0 }}>
                                                        ...
                                                        <div style={{ fontSize: '0.68rem', color: '#94A3B8' }}>
                                                            {CATEGORIA_LABEL[it.categoria]} · {Number(it.quantidadeAtual)}/{Number(it.quantidadeMinima)} {it.unidade}
                                                        </div>
                                                    </div>
                                                    {kpi === 'estoqueBaixo' && (
                                    ...
                                    {kpi === 'solicitacoesPendentes' && (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                            {(payload.pending?.length ?? 0) === 0 ? (
                                                ...
                                                <Link key={req.id} href={`/admin/estoque/solicitacoes`} style={{
                                                    display: 'flex', alignItems: 'center', gap: 10,
                                                    padding: '8px 10px', borderRadius: 9,
