...
export interface TurmaEstatisticasWorkspaceProps {
    classId: string;
    mode?: 'page' | 'drawer';
    onClose?: () => void;
    onBackToTurma?: () => void;
...
export default function TurmaEstatisticasWorkspace({
    classId: id,
    mode = 'page',
    onClose,
    onBackToTurma,
}: TurmaEstatisticasWorkspaceProps) {
    ...
    if (error || !cls) return (
        <div className="glass-card" style={{ textAlign: 'center', padding: '4rem' }}>
            ...
            <button
                type="button"
                onClick={() => (mode === 'drawer' && onClose ? onClose() : router.back())}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                {mode === 'drawer' && onClose ? (
                    <button
                        type="button"
                        onClick={onClose}
                        style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.78rem', color: '#9CA3AF', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer' }}
                    >
                        <ArrowLeftIcon style={{ width: 14, height: 14 }} /> Fechar
                    </button>
                ) : (
                ...
                {mode === 'drawer' && onBackToTurma && (
                    <>
                        <span style={{ color: '#D1D5DB', fontSize: '0.78rem' }}>/</span>
                        <button
                            type="button"
                            onClick={onBackToTurma}
                            style={{ fontSize: '0.78rem', color: '#9CA3AF', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
                        >
                            {cls.classIdentifier}
                        </button>
                    </>
