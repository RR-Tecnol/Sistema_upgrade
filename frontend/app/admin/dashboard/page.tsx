...
// DriverMarker type importado via re-export separado para não confundir o chunk boundary do webpack
// quando o mesmo módulo usa `import type` + `dynamic()` simultaneamente (bug Next.js 14 + webpack 5)
import type { DriverMarker } from '@/components/MapaMotoristas';

// Leaflet é browser-only — carregamento dinâmico obrigatório
// `.then(m => m)` explícito garante que o webpack resolve o chunk path corretamente
const MapaMotoristas = dynamic(
    () => import('@/components/MapaMotoristas').then(m => m),
    {
        ssr: false,
        loading: () => (
...
const DriverDrawer = dynamic(
    () => import('@/components/DriverDrawer').then(m => m),
    { ssr: false }
);
