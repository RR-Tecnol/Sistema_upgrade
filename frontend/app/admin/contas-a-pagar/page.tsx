        ...
        >
            ...
            <div style={{ position: 'relative', zIndex: 1, padding: '18px 18px 14px' }}>
                ...
                <div style={{ fontSize: '0.78rem', color: '#6B7280', lineHeight: 1.45, wordBreak: 'break-word', marginBottom: 12 }} title={primaryDescription}>
                    {/* Para compras de estoque: destaca nome do insumo + quantidade */}
                    {c.stockPurchaseRequest ? (
                        <span>
                            <strong style={{ color: '#0F172A' }}>{c.stockPurchaseRequest.stockItem.nome}</strong>
                            {' — '}
                            {Number(c.stockPurchaseRequest.quantidade)} {c.stockPurchaseRequest.stockItem.unidade}
                            {c.stockPurchaseRequest.fornecedor && (
                                <span style={{ color: '#94A3B8' }}> · {c.stockPurchaseRequest.fornecedor}</span>
                            )}
                        </span>
                    ) : (
                        primaryDescription
            ...
            <div style={{
                ...
                <button
                    ...
                    style={{
                        ...
                        padding: '10px 8px', borderRadius: 10, fontSize: '0.78rem', fontWeight: 700,
                        background: '#F9FAFB', border: `1px solid ${accentColor}30`,
                        color: '#374151', cursor: 'pointer',
                        transition: 'background .2s, border-color .2s',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5,
                    }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#F1F5F9'; e.currentTarget.style.borderColor = accentColor; }}
                    onMouseLeave={e => { e.currentTarget.style.background = '#F9FAFB'; e.currentTarget.style.borderColor = `${accentColor}30`; }}
                >
                    ✏️ Editar
                </button>
                {c.status === 'pendente' && (
                    <button
                        ...
                        style={{
                            ...
                            color: '#059669', fontSize: '0.78rem', fontWeight: 700,
...
// ⚠️ Antigo `ContaRastreioModal` removido a favor de `ContaPagarDetailModal`
// (em `components/admin/ContaPagarDetailModal.tsx`). O novo modal segue o
// padrão visual de Inscrições, exibe comprovante com lightbox + upload,
// vínculo cruzado com Solicitação de Compra / item de estoque e checklist
// binário de conformidade.

// (componente legado removido — ver `components/admin/ContaPagarDetailModal.tsx`)
            ...
            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
                ...
                {traceConta && (
                    <ContaPagarDetailModal
                        ...
                        onMarkAsPaid={async () => {
                            await handlePagar(traceConta.id);
                            setTraceConta(null);
                        }}
                        onDelete={() => {
                            setDeleteContaId(traceConta.id);
                            setDeleteContaDesc(traceConta.descricao);
                            setTraceConta(null);
                        }}
