...
import { CarretaDetailModal } from '@/components/admin/CarretaDetailModal';
...
function TruckCard({ truck, onDelete, onOpenDetail }: { truck: Truck; onDelete: (id: string) => void; onOpenDetail: (t: Truck) => void }) {
    const [hov, setHov] = useState(false);
    ...
    return (
        <div
            ...
            <div style={{ position: 'relative', zIndex: 1, padding: '18px 18px 14px' }}>
                ...
                {/* Botão Detalhes & Estoque — abre modal grande com tabs */}
                <button
                    type="button"
                    ...
                    onMouseEnter={e => {
                        e.currentTarget.style.background = `linear-gradient(135deg, ${accentColor}22, ${accentColor}10)`;
                        e.currentTarget.style.borderColor = `${accentColor}80`;
                        e.currentTarget.style.transform = 'translateY(-1px)';
                        e.currentTarget.style.boxShadow = `0 4px 14px ${accentColor}30`;
                    }}
                    onMouseLeave={e => {
                        e.currentTarget.style.background = `linear-gradient(135deg, ${accentColor}10, ${accentColor}05)`;
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                {loading ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '1.25rem' }}>
                        {filtered.map(truck => (
                            <TruckCard key={truck.id} truck={truck} onDelete={handleDelete} onOpenDetail={setDetailTruck} />
                        ))}
                    </div>
                )}
            </div>
