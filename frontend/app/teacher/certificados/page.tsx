...
export default function TeacherCertificados() {
    ...
    useEffect(() => {
        api.get('/certificates')
            .then(r => setCerts(Array.isArray(r.data) ? r.data : r.data?.data ?? []))
            .catch(() => setCerts([]))
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                <div>
                    ...
                    <p style={{ color: '#6B7280', fontSize: '0.85rem', margin: 0 }}>
                        Certificados das suas turmas — {certs.length} emitido{certs.length !== 1 ? 's' : ''}
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E5E7EB', padding: '4rem', textAlign: 'center' }}>
                    ...
                    <p style={{ color: '#9CA3AF', fontSize: '0.78rem', marginTop: '0.5rem' }}>
                        {search ? 'Tente outro filtro.' : 'Os certificados das suas turmas aparecerão aqui após a emissão.'}
                    </p>
                </div>
            ) : (
                        ...
                        <tbody>
                            {filtered.map((c, i) => (
                                <tr key={c.id} className="animate-fade-in" style={{ animationDelay: `${i * 30}ms` }}>
                                    <td style={{ fontWeight: 600, color: '#111827' }}>{c.studentName || '—'}</td>
                                    <td style={{ color: '#374151' }}>{c.courseName || '—'}</td>
                                    <td style={{ color: '#6B7280' }}>{c.className || '—'}</td>
                                    <td style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', color: '#6B7280', whiteSpace: 'nowrap' }}>{c.issuedAt ? fmtDate(c.issuedAt) : '—'}</td>
                                    <td style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.68rem', color: '#9CA3AF' }}>{c.protocol || '—'}</td>
                                    <td>
                                        {c.pdfUrl ? (
