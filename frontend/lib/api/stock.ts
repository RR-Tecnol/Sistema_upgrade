...
export interface StockItem {
    ...
    unidade: string;
    conteudoQuantidade?: number | string | null;
    conteudoUnidade?: string | null;
...
export interface StockMovement {
    ...
    observacao?: string | null;
    createdAt: string;
    stockItem?: { id: string; nome: string; unidade: string; precoUnitario?: number | string | null };
    fromTruck?: { id: string; identifier: string } | null;
...
// ── StockBudget (verba mensal por categoria) ───────────────────────
export interface StockBudget {
    ...
    /** Soma de PRs aprovadas/recebidas da mesma cat/mês criadas ANTES desta verba (sem stockBudgetId).
     *  Somado ao valorTeto na exibição da barra de consumo. */
    valorPreVerba?: number;
    observacao?: string | null;
    active: boolean;
    createdAt: string;
    updatedAt: string;
    categoriaCustom?: { id: string; nome: string; icon: string; color: string } | null;
    customCategory?: { id: string; nome: string; icon: string; color: string } | null;
}

export interface CreateStockBudgetDto {
...
export interface UpdateStockBudgetDto {
    valorTeto?: number;
    observacao?: string;
}
...
export const stockApi = {
    ...
    trucks: {
        ...
        updateMinimo: async (truckId: string, stockItemId: string, novoMinimo: number): Promise<TruckStockItem> => {
            const r = await api.patch<TruckStockItem>(`/stock/trucks/${truckId}/items/${stockItemId}/minimo`, { novoMinimo });
            return r.data;
        },
    },
    ...
    // ── Verbas mensais (StockBudget) ───────────────────────────────
    budgets: {
        list: async (filters?: { ano?: number; mes?: number; categoriaEnum?: string }): Promise<StockBudget[]> => {
            const params = new URLSearchParams();
            if (filters?.ano) params.append('ano', String(filters.ano));
            if (filters?.mes) params.append('mes', String(filters.mes));
            if (filters?.categoriaEnum) params.append('categoriaEnum', filters.categoriaEnum);
            const r = await api.get<StockBudget[]>(`/stock/budgets?${params.toString()}`);
            return r.data;
        },
        create: async (data: CreateStockBudgetDto): Promise<StockBudget> => {
            const r = await api.post<StockBudget>('/stock/budgets', data);
            return r.data;
        },
        update: async (id: string, data: UpdateStockBudgetDto): Promise<StockBudget> => {
            const r = await api.patch<StockBudget>(`/stock/budgets/${id}`, data);
            return r.data;
        },
        delete: async (id: string): Promise<void> => {
            await api.delete(`/stock/budgets/${id}`);
        },
    },
...
/**
 * Resolve o visual de uma categoria (label, ícone, cor) a partir de um objeto
 * que pode ser um StockItem (campo customCategory) ou um StockBudget (campo categoriaCustom).
 * Prioriza sempre a categoria customizada se ela existir.
 */
export function resolveCategoria(item: {
    categoria?: string | StockItemCategory;
    customCategory?: { nome: string; icon: string; color: string } | null;
    categoriaCustom?: { nome: string; icon: string; color: string } | null;
    categoriaEnum?: string | StockItemCategory | null;
}): {
    label: string;
    icon: string;
    color: string;
    ...
    // 1. Tenta pegar a categoria customizada (pode vir como 'customCategory' ou 'categoriaCustom')
    const custom = item.customCategory || item.categoriaCustom;
    if (custom) {
        return {
            label: custom.nome,
            icon: custom.icon || '❔',
    ...
    // 2. Fallback para o enum padrão (pode vir como 'categoria' ou 'categoriaEnum')
    const cat = (item.categoria || item.categoriaEnum || 'OUTRO') as StockItemCategory;
    return {
        label: CATEGORIA_LABEL[cat] || 'Outro',
        icon: CATEGORIA_ICON[cat] || '❔',
        color: CATEGORIA_COLOR[cat] || '#94A3B8',
        isCustom: false,
    };
}
...
/** Calcula o saldo global (Central + Todas as Carretas) */
export function getGlobalStockQuantity(item: StockItem): number {
    const central = Number(item.quantidadeAtual);
    const carretas = (item.truckStocks || []).reduce((acc, ts) => acc + Number(ts.quantidadeAtual), 0);
...
export function isLowStock(item: StockItem): boolean {
    ...
    return getGlobalStockQuantity(item) <= min;
}
...
export function isCriticalStock(item: StockItem): boolean {
    const min = Number(item.quantidadeMinima);
    if (min <= 0) return false;
    return getGlobalStockQuantity(item) <= min * 0.5;
}
...
export function getStockStatus(item: StockItem): StockStatus {
    const total = getGlobalStockQuantity(item);
    const minimo = Number(item.quantidadeMinima);
    if (minimo <= 0) return 'SEM_MINIMO';
    if (total <= minimo * 0.5) return 'CRITICO';
    if (total <= minimo) return 'BAIXO';
    return 'OK';
}
...
/**
 * Formata a quantidade de um item com a unidade de embalagem e, quando
 * disponível, o conteúdo por embalagem.
 *
 * Exemplos:
 *   formatItemQuantity(item, 10)  → "10 frascos × 250 mL"  (com conteudo)
 *   formatItemQuantity(item, 10)  → "10 un"                  (sem conteudo)
 *   formatItemQuantity(item, 10, { showTotal: true }) → "10 frascos × 250 mL (= 2.500 mL)"
 */
export function formatItemQuantity(
    item: Pick<StockItem, 'unidade' | 'conteudoQuantidade' | 'conteudoUnidade'>,
    quantidade: number | string,
    opts?: { showTotal?: boolean },
): string {
    const qtd = Number(quantidade);
    const base = `${qtd % 1 === 0 ? qtd : qtd.toFixed(3)} ${item.unidade}`;
    const cQtd = item.conteudoQuantidade != null ? Number(item.conteudoQuantidade) : null;
    const cUnd = item.conteudoUnidade ?? null;
    if (cQtd && cUnd) {
        const texto = `${base} × ${cQtd} ${cUnd}`;
        if (opts?.showTotal) {
            const total = qtd * cQtd;
            return `${texto} (= ${total % 1 === 0 ? total.toLocaleString('pt-BR') : total.toFixed(2)} ${cUnd})`;
        }
        return texto;
    }
    return base;
...
/**
 * Retorna só o rótulo do conteúdo por embalagem (ex: "250 mL", "5 kg").
 * Retorna null se não tiver conteúdo cadastrado.
 */
export function formatConteudoLabel(
    item: Pick<StockItem, 'conteudoQuantidade' | 'conteudoUnidade'>,
): string | null {
    if (!item.conteudoQuantidade || !item.conteudoUnidade) return null;
    const q = Number(item.conteudoQuantidade);
    return `${q % 1 === 0 ? q : q.toFixed(3)} ${item.conteudoUnidade}`;
}
